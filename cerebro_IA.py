...
if conexion_exitosa:
    ...
    # ====================================================================
    # PESTAÑA 4: VISUALIZADOR 3D SONDAJES (MOTOR MATEMÁTICO PURIFICADOR)
    # ====================================================================
            ...
            if sel_collar and sel_survey and sel_assay:
                with st.spinner(f"Procesando modelo 3D en tiempo real..."):
                    try:
                        ...
                        # 2. Limpieza de encabezados (Aspiradora de BOM invisible y espacios)
                        ...
                        # 4. Purificación de Datos (Forzar IDs a texto limpio y geometría a números)
                        ...
                        df_collar[c_x] = pd.to_numeric(df_collar[c_x], errors='coerce').fillna(0)
                        df_collar[c_y] = pd.to_numeric(df_collar[c_y], errors='coerce').fillna(0)
                        df_collar[c_z] = pd.to_numeric(df_collar[c_z], errors='coerce').fillna(0)
                        
                        df_survey[s_at] = pd.to_numeric(df_survey[s_at], errors='coerce').fillna(0)
                        df_survey[s_az] = pd.to_numeric(df_survey[s_az], errors='coerce').fillna(0)
                        df_survey[s_dip] = pd.to_numeric(df_survey[s_dip], errors='coerce').fillna(0)
                        
                        df_assay[a_from] = pd.to_numeric(df_assay[a_from], errors='coerce').fillna(0)
                        df_assay[a_to] = pd.to_numeric(df_assay[a_to], errors='coerce').fillna(0)
